package ru.night.util.render.backends.gl;
...
import ru.night.util.render.core.RenderFrameMetrics;
import ru.night.util.render.postfx.DepthRenderTarget;
import ru.night.util.render.postfx.DownsampleBlur;
...
public final class GlBackend {
   ...
   public GlBackend() {
      ...
      if (this.ssboSupported || instancedArraysSupported && drawInstancedSupported) {
         String vertexShaderPath = this.ssboSupported ? "assets/night/shaders/shape.vert"
               : "assets/night/shaders/shape_compat.vert";
         this.shapeProgram = ShaderProgram.fromResources(vertexShaderPath, "assets/night/shaders/shape.frag");
         ...
         this.screenBlur = new DownsampleBlur(32856, 5121);
         this.regionBlur = new DownsampleBlur(32856, 5121);
      } else {
   ...
   private ShaderProgram ensureFullscreenProgram() {
      if (this.fullscreenProgram != null) {
         ...
         String vertex = ResourceUtils.readText("assets/night/shaders/blur/blur_fullscreen.vert");
   ...
   public void warmupShaders() {
      // Сохраняем текущую программу Minecraft
      int previousProgram = org.lwjgl.opengl.GL11.glGetInteger(org.lwjgl.opengl.GL20.GL_CURRENT_PROGRAM);
      
      // Прогреваем шейдеры через glUseProgram + glGetUniformLocation.
      // На NVIDIA этого достаточно для триггера компиляции шейдера драйвером.
      // НЕ делаем glDrawArrays — это вызывало GL_INVALID_FRAMEBUFFER_OPERATION
      // и GL_INVALID_OPERATION ошибки, которые ломали состояние Minecraft рендера.
      
      if (this.shapeProgram != null) {
          this.shapeProgram.use();
      }
      if (this.fullscreenProgram != null) {
          this.fullscreenProgram.use();
      }
      if (this.screenBlur != null) this.screenBlur.warmup();
      if (this.regionBlur != null) this.regionBlur.warmup();
      
      // Восстанавливаем программу Minecraft
   ...
   public int createMsdfTexture(int width, int height, ByteBuffer data) {
      if (width <= 0 || height <= 0) {
         ...
         int prevAlignment = GL11.glGetInteger(3317);
         ...
         GL11.glPixelStorei(3317, prevAlignment);
      ...
      if (w > 0 && h > 0) {
         ...
         if (currentTex == 0 || w != currentW || h != currentH) {
            if (currentTex != 0) {
               ...
               // Dead code removed: int var9 = false;
            }

            if (currentFbo != 0) {
               GL30.glDeleteFramebuffers(currentFbo);
               // Dead code removed: int var11 = false;
            }
   ...
   public int captureRegionToTexture(int x, int y, int w, int h, boolean fullscreen) {
      ...
      try {
         ...
         GL11.glReadBuffer(s.readFramebuffer == 0 ? 1029 : 36064);
            ...
            try {
               ...
               if (client != null) {
                  ...
                  if (mainFramebuffer != null) {
                     int mainFramebufferTextureId = mainFramebuffer.getColorAttachment();
                     ...
                     GL30.glBindFramebuffer(36008, state.readFramebuffer);
                     GL11.glReadBuffer(state.readFramebuffer == 0 ? 1029 : 36064);
                  }
               } else {
   ...
   public GlBackend.FrameCapture captureFullFrame() {
      ...
      if (client == null) {
         ...
         if (framebuffer == null) {
            ...
            int sourceColor = framebuffer.getColorAttachment();
            int sourceDepth = framebuffer.getDepthAttachment();
