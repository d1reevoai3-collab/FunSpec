...
import ru.night.module.api.setting.impl.HueSetting;
import ru.night.ui.gui.GuiScreen;
import ru.night.ui.gui.component.setting.GuiRenderSetting;
import ru.night.util.color.ColorUtil;
import ru.night.util.keyboard.Keyboard;
import ru.night.util.player.MovementManager;
import ru.night.util.render.animation.util.Easings;
import ru.night.util.render.core.Renderer2D;
import ru.night.util.render.math.MathHelper;
import ru.night.util.render.math.animation.Direction;
import ru.night.util.render.text.FontRegistry;
import ru.night.util.render.utils.KeyUtil;
...
public class GuiRenderMain extends GuiScreen {
   public static void renderMain(Renderer2D renderer2D, MatrixStack pose, int mouseX, int mouseY, float mainAlpha) {
      ...
      if (ru.night.module.api.Category.Tickets.equals(GuiScreen.selectedCategories)) {
         ru.night.ui.gui.component.render.GuiRenderCustom.renderTickets(renderer2D, pose, mouseX, mouseY, mainAlpha);
         renderer2D.popClipRect();
         return;
      }
      if (ru.night.module.api.Category.History.equals(GuiScreen.selectedCategories)) {
         ru.night.ui.gui.component.render.GuiRenderCustom.renderHistory(renderer2D, pose, mouseX, mouseY, mainAlpha);
         renderer2D.popClipRect();
         return;
      }
      if (ru.night.module.api.Category.Logs.equals(GuiScreen.selectedCategories)) {
         ru.night.ui.gui.component.render.GuiRenderCustom.renderLogs(renderer2D, pose, mouseX, mouseY, mainAlpha);
         renderer2D.popClipRect();
         return;
      }
      ...
      float upMax = totalHeight + 150.0F;
      ...
      GuiScreen.getScrollUtil().setMax(MathHelper.clamp(upMax, 260.0F, 9999.0F), rectHeight - 10.0F);
      float yShar = -0.35F;
      float yZnar = -0.7F;
      int index = 1;
      ...
      for (Module module : filteredModules) {
         if (index % 2 == 0) {
            ...
            if (GuiScreen.openSettingsModules.contains(module) || settingsAnimx > 0.0F) {
               ...
               settingsHeightx = 12.0F * settingsAnimx + (fullSettingsHeightx - 12.0F) * settingsAnimx;
            }
            ...
            if (GuiScreen.openSettingsModules.contains(module) || settingsAnimxx > 0.0F) {
               ...
               settingsHeightxx = 12.0F * settingsAnimxx + (fullSettingsHeightxx - 12.0F) * settingsAnimxx;
            }
            ...
            if (!module.getSettingsForGUI().isEmpty() && !module.getSettingsForGUI().isEmpty()) {
               renderer2D.text(
                  FontRegistry.ICONS,
                  GuiScreen.x + 211.48F,
