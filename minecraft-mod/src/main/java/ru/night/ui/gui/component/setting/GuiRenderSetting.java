package ru.night.ui.gui.component.setting;
...
import ru.night.module.api.setting.impl.BindSettings;
...
import ru.night.module.api.setting.impl.NoneSetting;
...
import ru.night.util.render.math.animation.AnimationMath;
import ru.night.util.render.math.animation.anim.util.Animation2;
...
public class GuiRenderSetting {
   ...
   public static float renderSetting(
      ...
      if (setting instanceof NoneSetting) {
         ...
         sliderAnim.update();
         sliderAnim.run(targetProgress, 0.24F, ru.night.util.render.math.animation.anim.util.Easings.QUART_OUT);
         ...
         if (sliderSetting.percent) {
             valueText = sliderSetting.onlyInt ? String.format("%d%%", (int)sliderSetting.current) : String.format("%.1f%%", sliderSetting.current).replace(',', '.');
         } else {
             valueText = sliderSetting.onlyInt ? String.format("%d / %d", (int)sliderSetting.current, (int)sliderSetting.maximum) : String.format("%.1f / %.1f", sliderSetting.current, sliderSetting.maximum).replace(',', '.');
         }
